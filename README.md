...
面向内容创作团队的一站式智能运营平台：用 AI 拆解爆款、生成脚本与图文，统一管理矩阵账号的定时发布，并内置会员计费、算力结算与交付审核工作流。

## 三个端口

| 端 | 路径 | 面向角色 | 主要能力 |
|---|---|---|---|
| 用户端 | `/` | 客户公司运营员工 | AI 创作、爆款解析、视频创作、产品知识库、智能翻译、内容收藏/审核、发布计划、账号管理、每日签到与算力钱包 |
| 管理端 | `/manager/` | 客户公司负责人 | 运营总览、员工账号、算力充值与会员订阅（扫码支付+审核）、解析记录、算力流水 |
...
- **AI 创作助手**：个性化画像 + 模板库的对话式创作助手，一键产出图文/脚本
- **矩阵发布**：多社交平台账号扫码托管，图文/视频定时发布，独立 worker 执行队列，失败重试与安全策略
- **会员与算力**：四档会员方案，月度算力发放与次月清零、每日签到、到期自动回退免费版；租户级手动升降级
...
- **桌面端**：Electron（内嵌后端可选）
- **AI 集成**：多模态大模型、OpenAI 兼容接口，按租户可配
- **浏览器自动化**：patchright（账号扫码登录与发布）
...
- 已完成 ICP 备案并在页脚悬挂备案号（链接至工信部备案查询页）
- 公安联网备案按监管要求同步办理
