...
同时增加 `getCurrentUser()` 调用 `/api/auth/me`，并定义真实 `AuthUser` 角色类型。
门户不得继续作为所有人可见的演示切换器：`customer` 仅用户端，
`client_owner/client_admin` 可见用户端和管理端，
`platform_admin/developer_admin` 仅进入开发者端。客户角色的 DOM 和导航中不得出现
“开发者端”或 `AI 任务排查`。
...
- 显示“完成一次解析预计消耗 3 算力”。
- 默认提供一键“创建并开始解析”：依次创建任务、上传可选素材、运行解析；每个真实
  阶段分别显示创建中、上传中、解析中，不伪造百分比。任一步失败保留已创建任务并
  提供重试入口。
- 用户任务列表使用真实分页和请求序号防旧响应覆盖；运行、取消、保存草稿均在进行时
  禁用冲突操作。
- “转入灵感对话”必须真实创建一个 `goal_type="script"` 的灵感会话，把解析结果中的
  复用建议作为补充要求（按后端长度限制截断），创建成功后再导航到灵感对话页面。

- [ ] **Step 5: 实现管理和开发者页面**

管理端调用 Task 5 的服务端员工、时间、状态、关键词筛选和分页，显示完整只读结果。
开发者端调用服务端租户、状态筛选和分页，显示租户、用户、Provider、模型、耗时、
失败原因和调用状态；不显示 Key、完整 prompt 或未脱敏 Provider 原始响应。两端列表
与详情都必须防请求乱序；筛选后当前详情不在结果时清空或切换首条。

测试至少覆盖：客户看不到开发者入口、管理角色可进入管理端、开发者角色只进入开发者
端；FormData 不手工设置 Content-Type；一键创建/上传/运行顺序及中途失败重试；413
可读错误；三端分页/筛选/请求乱序；保存草稿幂等；转入灵感对话真实创建会话。
