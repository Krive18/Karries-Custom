...
export function ViralAnalysisPage({ onStartInspiration }: ViralAnalysisPageProps) {
  ...
  const writeSelectedDetail = useCallback((job: ViralAnalysisJob) => {
    if (selectedId.current === job.id) setDetail(job);
  }, []);
  ...
  const loadJobs = useCallback(async (requestedPage = 1) => {
    ...
    try {
      ...
      setJobs(result.items); setPage(result.page); setTotal(result.total);
      if (selectedId.current && !result.items.some((item) => item.id === selectedId.current)) { selectedId.current = null; ++detailRequest.current; setDetail(null); }
  ...
  const selectJob = useCallback(async (jobId: number) => {
    ...
    try {
      ...
      if (requestId === detailRequest.current && selectedId.current === jobId) { setDetail(job); setMessage(""); }
    } catch (error) { if (requestId === detailRequest.current && selectedId.current === jobId) setMessage(explainError(error)); }
  }, [isBusy]);
  ...
  async function createAndRun() {
    ...
    try {
      ...
      selectedId.current = created.id; ++detailRequest.current; setDetail(created);
      if (sourceType === "upload" && file) await uploadAndRun(created, file); else await runCreatedJob(created);
    ...
    finally { setOperation(null); await loadJobs(1); }
  }

  async function retryUpload() {
    if (!detail || pendingUploadJobId !== detail.id || !file || isBusy) return;
    setOperation({ phase: "uploading", jobId: detail.id });
    try { await uploadAndRun(detail, file); setMessage("素材上传并解析完成。"); }
    catch (error) { setMessage(explainError(error)); }
    finally { setOperation(null); await loadJobs(page); }
  }
  ...
  async function retry(job: ViralAnalysisJob) {
    ...
    try { const updated = await api.runViralAnalysisJob(job.id); writeSelectedDetail(updated); await loadJobs(page); }
  ...
  async function cancel(job: ViralAnalysisJob) {
    ...
    try { const updated = await api.cancelViralAnalysisJob(job.id); writeSelectedDetail(updated); await loadJobs(page); }
    catch (error) { setMessage(explainError(error)); }
    finally { setOperation(null); }
  }
